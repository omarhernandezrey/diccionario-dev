...
<html lang="es">
  ...
  base: `<!-- Define URL base para todas las rutas relativas -->
<head>
...
<body>
  ...
  slot: `<!-- archivo: my-component.js (Web Component) -->
...
async function ensureVariants() {
  try {
    console.log("🔧 Asegurando que TODOS los términos HTML tengan variantes...\n");

    let created = 0;
    let updated = 0;

    for (const [termName, snippet] of Object.entries(htmlTermsExamples)) {
      ...
      if (!term) {
        console.log(`⚠️  ${termName} no existe en BD`);
        continue;
      }

      // Verificar si ya existe variante
      const existingVariant = term.variants.find((v) => v.language === "html");
      ...
      if (existingVariant) {
        ...
        updated++;
        console.log(`✏️  ${termName}: variante actualizada`);
      } else {
        // Crear
        ...
        created++;
        console.log(`✅ ${termName}: variante creada`);
      }
    }

    console.log(
      `\n✨ Completado: ${created} creadas, ${updated} actualizadas`
    );
  } catch (error) {
