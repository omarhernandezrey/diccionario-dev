  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto w-full max-w-7xl 2xl:max-w-[1600px] px-4 sm:px-6 lg:px-8 2xl:px-12">
          ...
          <div className="space-y-4 w-full min-w-0">
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className={`text-xs font-bold mt-1 ${isNeo ? "text-neo-text-secondary" : "text-slate-600 dark:text-slate-400"}`}>IA semántica para shipping serio</p>
              </div>
            </div>

            {/* Description */}
            <p className={`text-sm font-medium leading-relaxed ${isNeo ? "text-neo-text-secondary" : "text-slate-700 dark:text-slate-400"}`}>
              Respuestas accionables con contexto bilingüe y snippets listos para producción.
            </p>

            {/* Feature Chips */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 2xl:gap-3 text-[11px]">
          ...
          <div className="w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {/* Primary CTA */}
	              <a
	                href="/"
	                className={`flex w-full items-center justify-center gap-2 rounded-2xl px-6 py-3.5 text-sm font-semibold text-white shadow-none dark:shadow-lg transition-all hover:-translate-y-0.5 ${isNeo
              ...
              <a
                href="#extensions"
                className={`flex w-full items-center justify-center gap-2 rounded-2xl border px-5 py-3 text-sm font-bold transition-all ${isNeo
              ...
              <a
                href="/api/docs"
                className={`flex w-full items-center justify-center gap-2 rounded-2xl border px-4 py-2.5 text-xs font-bold uppercase tracking-[0.14em] transition-all ${isNeo
        ...
        <div
          className={`mt-8 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 border-t pt-6 pb-6 text-sm font-medium ${isNeo ? "border-neo-border/30 text-neo-text-secondary" : "border-slate-200 dark:border-slate-800/70 text-slate-600 dark:text-slate-400"
            } items-center`}
        >
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-2 justify-center md:justify-start">
            <span className="flex items-center gap-2">
              <Heart className={`h-4 w-4 shrink-0 ${isNeo ? "text-neo-accent-pink" : "text-rose-400"}`} />
          ...
          <div className="flex items-center justify-center md:justify-end gap-3">
            <SocialLink variant={variant} href="https://github.com" label="GitHub" icon={<Github className="h-5 w-5" />} />
            <SocialLink variant={variant} href="https://twitter.com" label="Twitter" icon={<Twitter className="h-5 w-5" />} />
            <SocialLink variant={variant} href="https://linkedin.com" label="LinkedIn" icon={<Linkedin className="h-5 w-5" />} />
          </div>
        </div>
      </div>
    </footer>
...
function SocialLink({ variant = "neo", href, icon, label }: { variant?: FooterVariant; href: string; icon: React.ReactNode; label: string }) {
  ...
  return (
    <a
      href={href}
      aria-label={label}
      className={`flex h-10 w-10 items-center justify-center rounded-full border transition hover:-translate-y-0.5 ${isNeo
