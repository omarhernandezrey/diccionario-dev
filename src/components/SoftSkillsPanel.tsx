...
export default function SoftSkillsPanel({ tags }: SoftSkillsPanelProps) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-ink-900/50 p-4">
      <header className="flex items-center justify-between gap-2">
        <p className="text-xs uppercase tracking-wide text-white/60">Soft skills</p>
        {loading ? <span className="text-[11px] text-white/50">Carga…</span> : null}
      </header>
      {error ? <p className="mt-2 text-xs text-accent-danger">{error}</p> : null}
      <div className="mt-3 space-y-3">
        {items.map((template) => (
          <article key={template.id} className="rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            <div className="flex items-center justify-between gap-2">
              <strong className="text-white">{template.title}</strong>
              {template.scenario ? (
            ...
            <p className="mt-1 text-xs text-white/70">{template.questionEs}</p>
              ...
              <ul className="mt-2 space-y-1 text-[11px] text-white/60">
                {template.answerStructure.map((step) => (
                  <li key={`${template.id}-${step.title}`}>
                    <span className="font-semibold text-white/70">{step.title}:</span> {step.es}
                  </li>
                ))}
              </ul>
            ) : null}
            <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
              <button
                ...
                className="rounded-full border border-white/20 px-3 py-0.5 text-white/70 transition hover:border-white/40"
              >
                Copiar ES
              </button>
              <button
            ...
            {template.tipsEs ? <p className="mt-2 text-[11px] text-white/50">Tip: {template.tipsEs}</p> : null}
          </article>
        ))}
        {!items.length && !loading ? <p className="text-xs text-white/60">Aún no hay plantillas alineadas a esta búsqueda.</p> : null}
      </div>
    </section>
  );
}
