// (limpieza) Se eliminaron imports y duplicados iniciales.
...
describe('/api/terms GET', () => {
  it('400 si query inválida (page=0)', async () => {
    ...
    const { status } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms?page=0` });
    expect(status).toBe(400);
  });
  it('429 cuando rate limit bloquea', async () => {
    rateLimitMock.mockReturnValue({ ok: false, retryAfterMs: 1000, retryAfterSeconds: 1 });
    const mod = await importRoute('@/app/api/terms/route');
    const { status } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms` });
  ...
  it('200 devuelve items y meta', async () => {
    ...
    const mod = await importRoute('@/app/api/terms/route');
    const { status, json } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms?q=css` });
...
describe('/api/terms POST', () => {
  it('403 si requireAdmin retorna Response', async () => {
    ...
    const mod = await importRoute('@/app/api/terms/route');
    const { status } = await runRoute(mod.POST, { method: 'POST', url: `${baseUrl}/api/terms`, body: { term: 'x' } });
    expect(status).toBe(403);
  });
  it('400 si validación falla (term vacío)', async () => {
    const mod = await importRoute('@/app/api/terms/route');
    const { status } = await runRoute(mod.POST, { method: 'POST', url: `${baseUrl}/api/terms`, body: { term: '', translation: '' } });
    expect(status).toBe(400);
  });
  it('409 conflicto P2002', async () => {
    ...
    const { status } = await runRoute(mod.POST, { method: 'POST', url: `${baseUrl}/api/terms`, body });
  ...
  it('201 crea término y registra history', async () => {
    ...
    const { status, json } = await runRoute(mod.POST, { method: 'POST', url: `${baseUrl}/api/terms`, body });
...
describe('/api/terms/[id] GET', () => {
  it('400 id inválido (no numérico)', async () => {
    ...
    const { status } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms/abc`, params: { id: 'abc' } });
    expect(status).toBe(400);
  });
  it('429 rate limit', async () => {
    rateLimitMock.mockReturnValue({ ok: false, retryAfterMs: 1000, retryAfterSeconds: 1 });
    const mod = await importRoute('@/app/api/terms/[id]/route');
    const { status } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms/1`, params: { id: '1' } });
    expect(status).toBe(429);
  });
  it('404 no encontrado', async () => {
    prismaMock.term.findUnique.mockResolvedValueOnce(null);
    const mod = await importRoute('@/app/api/terms/[id]/route');
    const { status } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms/99`, params: { id: '99' } });
    expect(status).toBe(404);
  });
  it('200 devuelve término', async () => {
    prismaMock.term.findUnique.mockResolvedValueOnce({ id: 9, term: 'css', translation: 'css' });
    const mod = await importRoute('@/app/api/terms/[id]/route');
    const { status, json } = await runRoute(mod.GET, { url: `${baseUrl}/api/terms/9`, params: { id: '9' } });
    expect(status).toBe(200);
  const data = json as TermsApiResponse;
...
describe('/api/terms/[id] DELETE', () => {
  it('403 si requireAdmin falla', async () => {
    authMock.requireAdmin.mockImplementation(() => { throw new Response(null, { status: 403 }); });
    const mod = await importRoute('@/app/api/terms/[id]/route');
    const { status } = await runRoute(mod.DELETE, { method: 'DELETE', url: `${baseUrl}/api/terms/10`, params: { id: '10' } });
    expect(status).toBe(403);
  });
  it('404 si no existe para eliminar', async () => {
    prismaMock.term.findUnique.mockResolvedValueOnce(null);
    const mod = await importRoute('@/app/api/terms/[id]/route');
    const { status } = await runRoute(mod.DELETE, { method: 'DELETE', url: `${baseUrl}/api/terms/99`, params: { id: '99' } });
    expect(status).toBe(404);
  });
  it('200 elimina término', async () => {
    ...
    const mod = await importRoute('@/app/api/terms/[id]/route');
    const { status } = await runRoute(mod.DELETE, { method: 'DELETE', url: `${baseUrl}/api/terms/7`, params: { id: '7' } });
    expect(status).toBe(200);
  });
});
